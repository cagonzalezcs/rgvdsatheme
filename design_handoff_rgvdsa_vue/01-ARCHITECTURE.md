...
- **Events**: CPT `event` (title, start/end datetime, venue, city, category taxonomy, RSVP URL, description). Timber query → JSON → `EventCalendar` island props. Categories carry a color field matching the calendar filter swatches.
