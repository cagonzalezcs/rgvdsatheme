...
## Overview
Implementation handoff for the Rio Grande Valley DSA chapter website. Six screens were designed and prototyped: **Home**, **Get Involved**, **Calendar**, **Blog** (archive), **Blog Post** (single), and an **Interior Page Template** (used for Bylaws / Documents / governance-style pages). The target codebase is the existing `rgvdsatheme` WordPress theme (Timber/Twig + Vite via `@kucrut/vite-for-wp`).

The goal of this handoff: stand up **Vue 3 (Composition API) + Tailwind v4 + a project-owned shadcn-vue component library**, then implement the six designed screens with it.
...
4. **`designs/`** — the six HTML prototypes + runtime + logo assets.
