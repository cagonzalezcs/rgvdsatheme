...
Logo (`logo-red.png`, h 62px) · nav: **About ▾** dropdown (Mission & History, Counties We Serve, Committees, Bylaws & Code of Conduct, FAQ — cream panel, 3px ink border, 6px hard shadow, items hover `#9E0B13`/white), **Calendar**, **Blog**, **Get Involved** links (cream text, hover underline offset 4px) · right cluster: EN/ES toggle (2px cream border; EN active = cream fill/red text; ES disabled 0.65 opacity, title "Español — próximamente"), **Aa** a11y button, **Join DSA** CTA (cream fill, red text, uppercase, hover deep-red). Active page link = persistent underline + `aria-current="page"`.
...
`hello@example.org` email · Instagram `@dsa_rgv` (real) · WhatsApp invite links · subscribe URLs · the Who-We-Are photo · **all blog copy (titles, excerpts, prose, quotes, captions, author names) is intentionally lorem ipsum** · blog subscribe/RSS endpoints · Spanish translations ("Léelo en español" is a stub).
